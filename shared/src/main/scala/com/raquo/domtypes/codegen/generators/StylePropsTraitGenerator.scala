...
class StylePropsTraitGenerator(
  ...
  def hasSpecialTraits(implName: String): Boolean = {
    valueTraitsByImplName(implName).nonEmpty
  }
  ...
  override protected def printImplDefs(): Unit = {
    ...
    val customTypeImpls = allImplNames.filter(hasSpecialTraits)
    ...
    line()
    customTypeImpls.foreach { implName =>
      printImplDef(implName)
      line()
    }
  }
  ...
  def printImplDef(implName: String): Unit = {
    ...
    enter(List[String](
      ...
      line(implDefImpl(implName, valueTraits, typeParam = None))
    }
  }

  def implDefImplName(implName: String): String = s"new ${keyKind}"

  def implDefImpl(implName: String, valueTraits: List[String], typeParam: Option[String]): String = {
    List(
      s"${implDefImplName(implName)}",
      "(key)",
      traitTypeMixins(valueTraits, typeParam)
    ).mkString
  }
